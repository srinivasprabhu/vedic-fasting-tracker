  ...
  return (
    <Animated.View style={{ opacity: opac, transform: [{ translateY: slide }], paddingTop: SPACING.xl }}>
      ...
      <Text style={[s.sub, { color: mutedSub }]}>
        Scroll to your goal · where do you want to get to?
      </Text>
          ...
          <View style={s.previewRow}>
            ...
            <View style={[s.previewBadge, { backgroundColor: `${targetColor}22`, borderColor: `${targetColor}55` }]}>
              <Text style={[s.previewBadgeText, { color: targetColor }]}>
                {bmiCategoryLabel(targetCat)}{targetCat === 'normal' ? ' ✓' : ''}
              </Text>
            </View>
          </View>
        </View>
...
const s = StyleSheet.create({
  ...
  previewCard:     { borderWidth: 1.5, borderRadius: RADIUS.lg, padding: SPACING.md, marginTop: SPACING.sm } as ViewStyle,
  previewLabel:    { fontFamily: FONTS.bodyMedium, fontSize: 10, letterSpacing: .14, fontWeight: '500' as const, marginBottom: 8, textTransform: 'uppercase' as const } as TextStyle,
  ...
  previewBig:      { fontFamily: FONTS.displayLight, fontSize: 32, fontWeight: '300' as const, lineHeight: 36 } as TextStyle,
  previewUnit:     { fontFamily: FONTS.bodyMedium, fontSize: 11 } as TextStyle,
  previewBadge:    { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 10, borderWidth: 1 } as ViewStyle,
  previewBadgeText:{ fontFamily: FONTS.bodyMedium, fontSize: 11, fontWeight: '600' as const } as TextStyle,
});
